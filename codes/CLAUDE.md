...
- **Every chapter's charts are copies of the figures the compendium
  ...
  - Originals:
    ...
    - Still under the **original** compendium root, not V2's — only `codes\`
      was copied here, and nothing reads that folder at run time anyway.
...
- Paths live outside this repo, under
  `...\OneDrive - United Nations\Desktop\DSS\` — built from `Path.home()`,
  so they follow whichever account the folder sits under.
  - **This is V2**: everything the pipeline writes goes under
    `COMPENDIUM ARAB SOCIETY - V2\`, with the same folder names the original
    uses inside it. The questionnaires under `DATA COLLECTOR\` are read from
    the same place as before and never written to.
...
3. Translate them yourself. Use the official English name of a statistical
   body where one exists — search `translation dict_V2.xlsx` first, so
   wording stays consistent with what is already there.
...
- `translation dict_V2.xlsx` has four columns — `col_ar`, `val_ar`,
  `col_en`, `val_en` — plus `status`, which marks the rows the pipeline
  added.
  - **V2's own file, in the V2 root**, not the `translation dict.xlsx`
    under `DATA COLLECTOR` that the original pipeline reads and writes.
  ...
  - **Refuses a Source/citation row outright** — recognized by `col_ar`
...
- Excel locks files it has open.
  - Check `<Chapter>_*.xlsx` and `translation dict_V2.xlsx` are closed before a
